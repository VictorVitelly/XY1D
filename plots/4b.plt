...
do for [i=1:10] {
    ...
    column1[i]=i+1
    column2[i]=i+11
    fit g(x,ta,tb) '../data/corrfunc.dat' using 1:column1[i]:column2[i] every 1::197 via ta, tb
    tmpA[i]=ta
    tmpB[i]=tb
    tmpAerr[i]=ta_err
    tmpBerr[i]=tb_err
...
plot for [i=1:10] '../data/corrfunc.dat' using 1:column1[i]:column2[i] w errorbars linestyle i notitle ,for [i=1:10] g(x,tmpA[i],tmpB[i]) linestyle i title sprintf("T=%.2f, ξ=%.2f±%.3f, χ^2/dof=%.2f",TTT[i],tmpB[i],tmpBerr[i],chi2[i])
